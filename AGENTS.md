...
API、Service 与 DB 的业务归属、文件命名、导入方式、迁移流程和验收要求统一遵守 [`后端分层目录组织开发规约.md`](docs/后端分层目录组织开发规约.md)。

- `app/api/v1/`：HTTP 接口层，只负责请求解析、认证上下文、调用 Service 和响应转换；同一路由业务域的接口按目录归档，知识库主资源接口统一放在 `knowledge_base/`，平台 `/platform` 接口统一放在 `platform/`，具体结构遵守后端分层目录规约。
...
- 自主监控的告警、指标、证据、Agent 回答和其他客户可见内容必须优先使用指标定义表中的中文业务名称，禁止直接展示 `qa_error_rate`、`qa_success_rate` 等内部 `metric_code`；内部编码只允许保留在结构化元数据、查询条件、日志脱敏摘要和审计字段中。指标定义缺少中文名称时，客户可见内容统一显示“未配置中文名称”，不得回退显示内部编码。该约束必须在事实工具或 Service 数据组装层落实，不能只依赖 Prompt 要求模型翻译。
...
- 正式前端与交互原型中的常规界面文字必须统一复用全局 `--app-font-family` 字体变量，字体栈固定为 `"Microsoft YaHei", "PingFang SC", "Noto Sans CJK SC", "Source Han Sans SC", "Segoe UI", Arial, sans-serif`；业务页面和业务组件不得自行声明、复制或覆盖 `font-family`。
- 原生页面、Bootstrap、Element Plus 和 ECharts 必须绑定同一套全局字体；允许按视觉层级调整字号、字重和行高，但不得因模块、页面、组件或图表类型不同而切换字体族。
- 只有代码、SQL、JSON、Trace ID、规则表达式等具有明确代码语义的内容允许使用项目统一的等宽字体栈；其他文字不得以视觉装饰为由使用局部字体。
- 全局字体栈发生调整时，必须依次同步交互原型、设计与开发规约、全局主题、Element Plus/Bootstrap 字体变量、ECharts 主题和对应测试用例，禁止只修改单个页面。
