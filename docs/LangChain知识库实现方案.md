...
系统整体采用 RAG（Retrieval-Augmented Generation，检索增强生成）架构，完整覆盖：
...
| 向量数据库 | PostgreSQL + pgvector | PostgreSQL `16.x`；pgvector `0.8.x` | 同时承担关系数据和向量数据存储 |
...
| 文件存储 | MinIO | `RELEASE.2025-09-07T16-13-09Z` | 保存原始文件和解析后的中间文件；生产环境按部署规模扩展 |
...
普通文档默认使用 `chunk_size=600`、`chunk_overlap=100`；对制度、手册和长篇结构化文档，同时支持 Parent-Child 结构，以便在命中子片段后恢复完整父级上下文。

### 5.4 检索策略

建议采用两阶段检索：

- 召回阶段：向量检索 + 关键词检索，召回 20~50 个候选片段。
- 排序阶段：使用 Cross-Encoder 或模型重排序，保留 5~10 个高相关片段。
...
项目基线使用 PostgreSQL + pgvector、异步数据库访问和 Arq/Redis 索引任务，因此部署环境应安装 `databases[postgresql]`、`asyncpg`、`pgvector`、`psycopg[binary]`、`arq` 和 `redis`。如果部署时选择 Qdrant、Milvus 或其他向量数据库，应替换对应适配层，而不是同时安装所有向量数据库驱动。
...
- 完成 Vue 3 前端、FastAPI 后端和统一配置体系。
- 完成 PostgreSQL、pgvector、Redis、MinIO 和任务调度基础设施。
- 完成知识库、文档、分块、会话、消息和引用数据模型。
- 完成日志、异常、认证上下文和请求追踪。

### 阶段二：文档接入与检索

- 支持 PDF、Word、Markdown、TXT、网页和表格解析。
- 完成结构化切分、Parent-Child 分块和 Embedding 入库。
- 完成向量检索、关键词检索、混合检索和重排序。
- 完成异步索引、失败重试、增量更新和索引版本管理。
- 完成知识库权限、租户隔离和检索阶段 ACL 过滤。

### 阶段三：问答与知识应用

- 完成会话、多轮问题改写、上下文组装和引用返回。
- 完成 OCR、表格理解、图片和多模态内容接入。
- 完成模型路由、缓存、流式输出、限流和成本控制。
- 完成管理后台、问答反馈、评测集和人工审核流程。

### 阶段四：生产化运营

- 完成多租户细粒度权限、组织和成员管理。
- 完成索引版本灰度切换、回滚和数据生命周期管理。
- 完成高可用部署、扩容、备份、监控和故障恢复。
- 完成评测集自动回归、质量看板和模型效果对比。
...
实施时应以真实业务文档和完整链路为基准，依次推进：上传文件 → 解析切分 → 向量入库 → 混合检索与重排序 → 带引用问答 → 多轮会话 → 权限过滤 → 固定评测与生产化运营。各阶段是实施顺序，不代表对目标能力进行裁剪。
