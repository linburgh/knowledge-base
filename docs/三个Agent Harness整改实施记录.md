...
2026年08月09日确认结构化终态缺失是三个 Agent 的共性供应商兼容风险：知识问答复杂模式、自主监控和自主评测均使用 `ToolStrategy`，此前在 `structured_response` 缺失时直接进入确定性收敛。现已增加统一的受限终态修复协议：首次终态缺失或 Schema 校验失败后，使用官方 `create_agent` 和目标 `ToolStrategy` 再调用模型一次；修复 Agent 不注册任何业务工具，模型调用上限为一次，并受原 Harness 的剩余时间和模型调用预算限制。

三个 Harness 只复用通用修复协议，不共享模型实例、Runtime、Policy、Registry 或事实组装。知识问答传入已检索分块并继续校验引用，监控传入可信窗口和已取得事实并继续执行确定性结论校验，评测传入逐题状态和确定性指标并继续以门禁结果为准。修复仍缺失、非法、超时或供应商失败时保持原确定性收敛，且不会重新执行检索、监控查询或评测题目。

专项自动化新增成功修复、再次缺失、无剩余预算、修复超时和业务工具不重复调用场景。三个 Agent 及关联监控、评测专项共 81 项通过，后端全量回归 274 项通过，Ruff、Python 编译和差异格式检查通过。随后使用当前真实外部模型供应商并行执行三个脱敏 Schema 能力探针，知识问答 `AgentAnswer`、自主监控 `MonitoringAgentOutput` 和自主评测 `EvaluationAgentOutput` 均在受限修复轮一次成功，未进入再次降级。探针不访问数据库、不发送真实问题或业务事实，只验证当前供应商对单一终态工具的协议兼容性；完整业务端到端仍按各 Agent 的真实链路测试单独验收。
