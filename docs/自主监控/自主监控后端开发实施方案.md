...
| `GET` | `/api/v1/monitoring/metrics` | 指标趋势、状态和明细 |
| `GET` | `/api/v1/monitoring/tasks` | 任务、积压和 Worker 状态 |
| `GET` | `/api/v1/monitoring/evaluations` | 评测运行和 Agent 执行证据 |
| `GET` | `/api/v1/monitoring/events` | 事件查询和时间线 |

### 7.2 告警接口

| 方法 | 路径 | 用途 |
| --- | --- | --- |
| `GET` | `/api/v1/monitoring/alerts` | 告警分页查询 |
...
| `GET` | `/api/v1/monitoring/notifications/records` | 查询发送记录 |
