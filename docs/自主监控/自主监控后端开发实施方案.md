...
└── workers/
    └── monitoring/
        ├── __init__.py
        ├── collect.py           # 周期状态和主动探针采集
        ├── aggregate.py         # 指标聚合并调用规则引擎
        └── notify.py            # 通知发送和失败重试
```
...
| 总览、列表和详情编排 | `app/core/services/monitoring/mgr.py` | 已规划，需新增 |
| 指标规则引擎 | `app/core/services/monitoring/rule.py` | 已补充，需新增 |
| 分析总览和分析对话 | `app/core/services/monitoring/analysis.py` | 已规划，需新增 |
| 通知编排 | `app/core/services/monitoring/notification.py` | 已规划，需新增 |
| 权限范围 | `app/core/services/monitoring/access.py` | 已规划，需新增 |
| 监控数据 Repository | `app/db/monitoring/*.py` | 已按数据表逐表补齐 |
| 通用会话 Repository | `app/db/knowledge_base/conversation.py`、`app/db/knowledge_base/conversation_message.py` | 复用，不新增分析会话表 |
| 审计 Repository | `app/db/platform/audit_log.py` | 复用，不新增监控专用审计表 |
| 周期采集 | `app/workers/monitoring/collect.py` | 已规划，需新增 |
| 指标聚合和规则执行 | `app/workers/monitoring/aggregate.py` + `app/core/services/monitoring/rule.py` | 已明确职责边界 |
| 通知发送 | `app/workers/monitoring/notify.py` | 已规划，需新增 |

其中，`workers/monitoring/aggregate.py` 只是 Worker 执行入口，不承载规则业务逻辑；规则判断统一放在 `app/core/services/monitoring/rule.py`，供 Worker、接口和测试复用。
...
| 采集方式 | 目标编码 | 代码入口或探针范围 |
| --- | --- | --- |
| 方法采集 | `knowledge.qa` | `app.agents.knowledge.agent.run` |
| 方法采集 | `evaluation.run` | `app.workers.evaluation.run_evaluation` |
| 方法采集 | `document.ingestion` | `app.core.services.knowledge_base.document.upload` |
| 方法采集 | `document.indexing` | `app.core.services.knowledge_base.ingestion.run_claimed_task` |
...
- 分析对话由受限版 `create_deep_agent` 执行模型—工具循环，模型根据中间证据自主继续查询或结束；程序根据实际工具结果确定结论、事实、限制和授权范围，模型不能改变确定性事实判断。
- 回答编排支持标题、引用块、强调、列表、编号步骤和紧凑表格，结构按问题动态选择；调用失败或校验失败时使用确定性中文 Markdown 降级回答。
- 所有客户可见时间统一表述为“中国标准时间”，内部时区计算继续使用稳定时区编码。
- 告警事件的客户可见资源名称必须从授权租户信息解析；租户名称不可用时回退“租户 + ID”，平台范围回退“全平台”。内部 `alert_key`、`scope_key` 和 `source_code` 仅用于关联与审计，不得直接作为影响对象返回。
...
- Monitoring Agent 使用自身模型实例和 `create_deep_agent` 结构化输出，不导入知识库 Agent 或评测 Agent 的私有模型、Prompt 或 Runtime。工具选择、观察和终止由官方 Agent 循环执行，Policies 与 Runtime 负责最终授权、范围、单工具超时和轨迹收集。
- 本地时间解析仅用于模型没有调用事实工具时的确定性元数据降级，不再以关键词 Planner 作为生产 Agent 主链。
...
- `section_statuses`：按 `unresolved_alerts`、`runtime_status`、`business_status`、`alert_status_trend`（兼容键，当前承载告警状态概览）、`resource_capacity`、`propagation` 返回 `ready`、`empty`、`partial` 或 `error`。总览同时返回 `alert_status_summary` 四类状态统计和 `recent_alert_changes` 最近 3 条真实变化。
- `section_errors`：仅为查询失败或部分失败的模块返回稳定、脱敏的错误说明，不返回 SQL、参数、异常堆栈和底层连接信息。
- 告警摘要和告警状态概览依赖告警数据；业务状态和链路状态依赖事件数据；资源容量依赖运行快照。告警状态数字和 `recent_alert_changes` 均保留所有未恢复告警，历史告警只统计所选窗口内的变化；`recent_alert_changes` 按候选告警的最近状态时间倒序并最多返回 3 条，不得在存在活动告警时返回空列表。
...
- 接口必须接收前端的 `time_range`，支持 `15m`、`1h`、`6h`、`24h`、`7d`，分别过滤事件 `occurred_at`、告警最近状态时间和状态快照 `updated_at`。窗口外历史数据不得继续作为当前总览数据展示，但窗口外仍未恢复的活动告警作为风险提示继续计入当前状态数字和“最近变化”候选，避免统计有值但告警列表为空。
...
- `GET /api/v1/monitoring/tasks/overview` 返回最新任务状态、最老等待时长和最新 Worker 状态；趋势按查询窗口补齐连续的 5 分钟时间桶，`pending_backlog` 表示桶末待处理存量，`created/completed/failed/timeout` 表示桶内流量，取消任务不并入失败。任务列表不混入 Worker 心跳。
