...
与自主监控的边界：自主评测 Agent 只负责评测任务编排、评测结果和执行证据；自主监控 Agent 负责读取这些结构化数据，生成监控分析总览和分析对话。分析对话不属于自主评测 Agent，分析会话历史也不写入评测任务、评测运行或评测结果表。
...
| `validate_task`       | 校验授权用户发起的运行、知识库和配置 | 配置错误或越出租户则不进入问答阶段 |
...
评测 Agent 除了生成评测报告，还必须根据指标结果、失败样品和配置快照生成优化建议。优化建议不是新的评分指标，而是对评测结果的分析结论，至少包含：

- 问题现象：哪个指标或哪类问题未达到预期。
- 数据证据：实际值、门禁值、失败数量和关联问题编号。
- 优化建议：建议调整的参数、Prompt、检索链路或文档处理环节。
- 预期影响：可能改善的指标。
- 优先级：高、中、低。
- 建议置信度：高、中、低。
- 是否需要人工确认：涉及配置发布、索引重建或文档修改时必须需要确认。

优化建议的边界：

- Agent 只分析问题并提出建议，不自动修改 `app.yaml` 或数据库配置。
- Agent 不自动发布问答配置，不自动重建索引。
- 涉及文档归属、知识库范围或业务规则的建议必须标记为人工复核。
- 当关键指标无法计算时，只输出数据或链路补充建议，不虚构参数优化结论。

示例：

```json
{
  "issue": "Recall@5 低于门禁要求",
  "evidence": {
    "value": 0.8,
    "threshold": 0.9,
    "failed_case_ids": ["case-003", "case-008"]
  },
  "suggestion": "检查文档分块边界，并对 chunk_size=600 与 chunk_size=400 进行对比评测",
  "affected_metrics": ["Recall@5", "Context Recall"],
  "priority": "high",
  "confidence": "medium",
...
在评测报告的优化建议旁增加“帮我优化”操作。该能力用于根据评测结果生成候选优化方案，并通过同一批测试问题重新评测优化效果。

“帮我优化”不是直接修改配置的一键操作，而是一个可审查、可对比、需确认的优化流程：
...
```text
当前配置：chunk_size = 600，retrieval_top_k = 5
候选方案：chunk_size = 400，retrieval_top_k = 8
...
- 默认只生成候选配置，不直接修改正式配置。
- 不自动发布问答配置，必须由用户确认后执行。
- 不自动修改文档内容、知识库范围或业务规则。
- 涉及分块参数、文档处理参数等索引相关配置时，必须明确提示需要重建索引。
- 复测指标未提升、关键指标无法计算或出现退化时，不得标记为“建议采用”。
- 用户确认后支持“保存为草稿”；“发布配置”仍遵循现有版本、权限和索引切换流程。
...
- 评测 Agent 使用数据库用户主键 `t_user.id` 作为评测用户；未指定时默认使用当前已登录用户 ID。租户角色显式指定评测用户时，必须校验该用户属于当前租户；所有显式用户都必须存在且状态有效，不能使用不存在的账号占位符。
...
在“平台管理”下新增“自主评测”菜单，用于平台超级管理员或当前租户的 `tenant_admin` 维护评测任务并查看每次评测结果。该模块是知识库问答评测 Agent 的管理入口，不改变现有知识库问答入口，也不把评测逻辑放入知识库问答 Agent。
...
普通平台角色、`tenant_member`、组织角色和访客均不可看到该菜单，也不可调用该模块接口。`tenant_admin` 只能看到和操作当前租户的评测任务。权限判断必须以后端当前用户的有效角色和租户范围为最终依据，前端菜单隐藏和 `v-permission` 只作为交互层控制，不能替代后端鉴权。
...
| 层级           | 编码                   | 名称     | 路径                    | 可见角色           |
| -------------- | ---------------------- | -------- | ----------------------- | ------------------ |
| 平台管理子菜单 | `platform_evaluations` | 自主评测 | `/platform/evaluations` | `p_super_admin`、`tenant_admin`；租户角色限当前租户 |
...
这些操作授权给 `p_super_admin`、`tenant_admin`。即使未来菜单授权配置发生变化，Service 仍须额外校验当前用户角色和当前租户数据范围，形成双重保护。
...
主列表参考“参考证据”列表采用四列简约结构：评测信息（评测名称、知识库名称、问题来源、问题数量）、执行结果（最新执行状态、最新结论）、时间人员（最近执行时间、创建人、创建时间）和操作入口。查询字段仍对应组合列中的业务信息，不因减少展示列而减少筛选能力；完整运行配置和结果进入详情抽屉查看。
...
2. 总体结论：通过、不通过或无法判定。
3. 指标卡片：指标中文名称、实际值、门禁值、比较符、样本数和是否合格；前端不展示英文标识。
...
7. 优化建议：展示问题证据、建议措施、影响指标、优先级、置信度和是否需要人工确认。
8. “帮我优化”：根据优化建议生成候选配置，启动同批问题复测，并展示优化前后指标对比；复测完成后由用户选择保存草稿或结束本次优化。

逐题答案和参考资料可能较长，详情区域必须限制高度并提供滚动条；列表中的长文本单行省略，完整内容通过详情或 Tooltip 查看。

### 12.5 后端接口设计

接口统一挂载在 `/api/v1/platform/evaluations`，所有接口均要求登录并由 Service 层校验角色和租户范围：
...
| `tenant_id`                  | `bigint`                                  |   是 | 所属租户 ID，必须与目标知识库的租户一致                              |
...
- `idx_t_evaluation_task_tenant`：`(tenant_id, status, created_at)`，按租户隔离并分页查询任务。
...
路由守卫仍需验证菜单访问权；后端接口必须再次验证角色和租户范围，不能仅依赖前端指令或路由隐藏。
...
- 新增 `platform_evaluations` 菜单、`evaluation:*` 页面操作，并为 `p_super_admin`、`tenant_admin` 建立授权关系；租户角色限制当前租户。
- 新增评测任务、评测运行和结果明细的数据模型及 DDL/DML。
- 新增任务分页、创建、执行、删除和结果查询接口。
- 新增平台管理下的自主评测列表页、任务表单和结果详情页。
- 前端使用 `v-permission` 控制按钮显示，后端 Service 额外校验角色和租户范围。
- 执行中的任务禁止重复执行和删除，历史运行结果只读保存。

验收：平台超级管理员和当前租户的 `tenant_admin` 可以完成“新增 → 执行 → 查看结果 → 删除”闭环；其他角色访问菜单和接口均被拒绝，租户角色不能访问其他租户任务。
...
10. `p_super_admin`、`tenant_admin` 可以新增、执行、删除和查看自主评测；租户角色仅限当前租户，前后端权限表现一致。
