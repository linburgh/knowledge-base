...
| 模型未提交结构化终态 | 已取得事实被直接降级，正常结果率下降 | 首次缺失或 Schema 校验失败后执行一次无业务工具的受限终态修复；再次失败才确定性收敛 |

### 终态修复

三个 Agent 统一采用“首次结构化提交、一次受限修复、确定性收敛”的终态策略。受限修复使用 LangChain 官方 `create_agent` 与 `ToolStrategy`，只注册目标 Schema 生成的结构化终态工具，不注册知识检索、监控查询或评测执行工具，模型调用上限固定为一次，并服从原 Agent 的剩余总时限和模型调用预算。

修复输入只包含原 Agent 已取得的授权事实摘要。知识问答使用问题和已检索分块，监控分析使用可信时间范围和监控事实，评测使用逐题状态与确定性指标；不得仅解析首次自由文本并将其冒充事实。修复结果仍须经过各 Agent 自己的引用、结论、权限和 Schema 校验。修复超时、供应商异常、结构仍缺失或字段仍非法时，继续使用原 Harness 的确定性收敛，不重复业务查询，不丢失工具轨迹。

共性代码只封装无工具结构修复协议，不共享三个 Agent 的 Runtime、Policy、Registry、模型实例或业务事实组装逻辑。每个 Harness 独立决定剩余预算、输入摘要、领域校验和最终降级结果，以保持 Agent 隔离。
...
- 首次终态缺失或 Schema 非法时最多修复一次，修复轮不含业务工具；修复失败后仍能确定性收敛。
