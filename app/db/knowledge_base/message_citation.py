...
async def insert_(db, **kwargs: Any) -> Any:
    return await db_api.insert_(db, MessageCitation, **kwargs)


async def update_(db, values: dict[str, Any], **kwargs: Any) -> Any:
    return await db_api.update_(db, MessageCitation, values, **kwargs)


async def delete_(db, **kwargs: Any) -> Any:
    return await db_api.delete_(db, MessageCitation, **kwargs)


async def get(db, **kwargs: Any) -> dict[str, Any] | None:
    return await db_api.get(db, MessageCitation, **kwargs)
...
async def list(db, **kwargs: Any) -> list[dict[str, Any]]:
    return await db_api.list(
        db,
        MessageCitation,
        order_by=[MessageCitation.c.rank.asc(), MessageCitation.c.id.asc()],
        **kwargs,
    )
