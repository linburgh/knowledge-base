...
def build_evaluation_chat_model() -> ChatOpenAI:
    """构建评测 Agent 自己的模型，不复用知识库 Agent 的私有模型实例。"""
    if not CONF.chat.model:
        raise BusiException("评测 Agent 对话模型未配置")
    if not CONF.chat.api_key:
        raise BusiException("评测 Agent 对话模型密钥未配置")
...
__all__ = ("build_evaluation_chat_model",)
